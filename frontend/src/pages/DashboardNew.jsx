...
export default function Dashboard() {
  ...
  const metrics = useMemo(
    () => ({
      upcoming: upcomingAppointments.filter(
        (apt) => !isBefore(new Date(apt.scheduled_at), new Date()),
      ).length,
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">
            Referral Follow-Up Dashboard
          </h1>
          <p className="text-gray-600 mt-2">
            Track missed appointments, agent outreach, and follow-ups
          </p>
        </div>
        <button
        ...
        <div className="bg-red-50 border-l-4 border-red-500 rounded-lg p-6 mb-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-xl font-bold text-gray-900">
              Flagged Patients – Nurse Action Required
            </h2>
          </div>
          <p className="text-sm text-gray-700 mb-6">
            These patients require immediate attention due to failed contact
            attempts or rescheduling issues.
          </p>

          <div className="space-y-4">
            {flaggedPatients.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                No flagged patients at this time
              </div>
            ) : (
              flaggedPatients.map((flag) => {
                ...
                return (
                  <div
                    key={flag.id}
                    className="bg-white rounded-lg border-2 border-red-200 p-6 shadow-sm"
                  >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm text-gray-600 mb-4">
                          <div>
                            ...
                            {flag.reason || 'No answer / Multiple attempts'}
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            {flag.flag_type || 'Follow-up required'}
                          </div>
                          <div>
                            <span className="font-medium">Call Attempts:</span>{' '}
                            {flag.call_attempts || 3}
                          </div>
                        </div>
                      </div>
                    </div>
      ...
      <section className="mb-8">
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
          ...
          <p className="text-sm text-gray-600 mb-6">
            Patients who missed appointments and are eligible for AI agent
            outreach.
          </p>
          ...
          <div className="space-y-4">
            {missedAppointments.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                No missed appointments requiring follow-up
              </div>
            ) : (
              missedAppointments.slice(0, 3).map((apt) => {
                ...
                return (
                  <div
                    key={apt.id}
                    className="bg-orange-50 rounded-lg border border-orange-200 p-6"
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="grid grid-cols-3 gap-4 text-sm text-gray-600 mb-4">
                          <div>
                            <span className="font-medium">Missed Date:</span>
                            <br />
                            {apt.scheduled_at
                              ? format(
              ...
              <tbody>
                {upcomingAppointments.length === 0 ? (
                  ...
                  upcomingAppointments.slice(0, 10).map((apt) => {
                    ...
                    const daysUntil = apt.scheduled_at
                      ? differenceInDays(new Date(apt.scheduled_at), new Date())
                      : 0
